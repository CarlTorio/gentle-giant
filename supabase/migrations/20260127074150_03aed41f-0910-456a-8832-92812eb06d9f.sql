...
ALTER TABLE public.bookings ADD CONSTRAINT fk_bookings_patient FOREIGN KEY (patient_id) REFERENCES public.patient_records(id) ON DELETE SET NULL;
...
CREATE TRIGGER generate_member_referral_code BEFORE INSERT ON public.members FOR EACH ROW EXECUTE FUNCTION public.generate_referral_code();
...
CREATE TRIGGER trigger_sync_member_to_patient AFTER UPDATE ON public.members FOR EACH ROW EXECUTE FUNCTION public.sync_member_to_patient();
