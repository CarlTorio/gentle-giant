...
BEGIN
  IF NEW.status = 'active' AND (OLD IS NULL OR OLD.status != 'active') THEN
    ...
    ELSE
      ...
      SET 
        name = NEW.name, email = NEW.email, contact_number = NEW.phone,
        membership = NEW.membership_type, membership_join_date = NEW.membership_start_date,
        membership_expiry_date = NEW.membership_expiry_date, membership_status = NEW.status,
        payment_method = NEW.payment_method, payment_status = NEW.payment_status,
        amount_paid = NEW.amount_paid, stripe_customer_id = NEW.stripe_customer_id,
        stripe_payment_intent_id = NEW.stripe_payment_intent_id,
        stripe_receipt_url = NEW.stripe_receipt_url, updated_at = now()
  ...
  IF NEW.status = 'active' AND OLD IS NOT NULL AND OLD.status = 'active' THEN
    UPDATE public.patient_records
    SET name = NEW.name, email = NEW.email, contact_number = NEW.phone,
        membership = NEW.membership_type, membership_join_date = NEW.membership_start_date,
        membership_expiry_date = NEW.membership_expiry_date, membership_status = NEW.status,
        updated_at = now()
    WHERE member_id = NEW.id;
