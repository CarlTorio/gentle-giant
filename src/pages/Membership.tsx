...
const Membership = () => {
  ...
  const handlePayNow = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate referral code for display (will be auto-generated by DB trigger)
      const displayReferralCode = newMember?.referral_code || 
        formData.name.replace(/[^a-zA-Z]/g, '').toUpperCase().substring(0, 6).padEnd(6, 'X');
      
      // Send membership confirmation emails (non-blocking)
      try {
        await supabase.functions.invoke('send-membership-confirmation', {
          body: {
            ...
            membershipTier: selectedPlanData?.name || 'Green',
            referralCode: displayReferralCode,
            amountPaid: selectedPlanData?.price || 0
          }
        });
      } catch (emailErr) {
            ...
            duration: 0.6
              ...
              duration: 0.5
                ...
                <div className="flex items-center gap-3 mb-4 pb-4 border-b border-border">
                  <img src={selectedPlan?.image} alt={selectedPlan?.name} className="w-16 h-auto rounded-lg" />
                  <div>
      ...
      <main className="pt-20 pb-10">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          duration: 0.4
            ...
            <Button variant="ghost" onClick={() => navigate(-1)} className="text-muted-foreground hover:text-foreground gap-2 h-10 px-3 text-sm md:text-base">
          ...
          <div className="grid lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-3">
              <form onSubmit={handlePayNow} className="space-y-5">
                ...
                delay: 0.1
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input id="name" name="name" placeholder="Enter your full name" value={formData.name} onChange={handleInputChange} className={`h-11 text-base ${errors.name ? "border-destructive" : ""}`} />
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input id="contact" name="contact" placeholder="Enter your contact number" value={formData.contact} onChange={handleInputChange} className={`h-11 text-base ${errors.contact ? "border-destructive" : ""}`} />
                    ...
                    <div className="space-y-1.5 md:col-span-2">
                      ...
                      <Input id="email" name="email" type="email" placeholder="Enter your email address" value={formData.email} onChange={handleInputChange} className={`h-11 text-base ${errors.email ? "border-destructive" : ""}`} />
                    ...
                    <div className="space-y-1.5 md:col-span-2">
                      ...
                      <Input id="referralCode" name="referralCode" placeholder="Enter a 6-character referral code" value={formData.referralCode} onChange={handleInputChange} maxLength={6} className={`h-11 text-base uppercase ${errors.referralCode ? "border-destructive" : ""}`} />
                ...
                delay: 0.2
                  ...
                  if (errors.membership) {
                    ...
                    scale: 0.98
                        ...
                        <Label htmlFor={membership.id} className={`cursor-pointer block rounded-lg p-3 border-2 transition-all ${selectedMembership === membership.id ? "border-accent bg-accent/5 shadow-md" : "border-border hover:border-accent/50"}`}>
                ...
                <div className="lg:hidden">
                  <OrderSummary selectedPlan={selectedPlan} isProcessing={isProcessing} onPayNow={() => {}} isMobile />
                </div>
                ...
                delay: 0.3
                  ...
                  <Button type="submit" disabled={isProcessing || !selectedMembership} className="w-full gradient-accent text-accent-foreground rounded-full h-12 text-base font-semibold disabled:opacity-50 gap-2">
            ...
            <div className="hidden lg:block lg:col-span-2">
              ...
              delay: 0.3
                ...
                <OrderSummary selectedPlan={selectedPlan} isProcessing={isProcessing} onPayNow={handlePayNow} />
              </motion.div>
            </div>
          </div>
        </div>
...
const OrderSummary = ({
  ...
  return <div className={`bg-card rounded-xl p-5 shadow-lg border border-border/50 ${isMobile ? '' : ''}`}>
      ...
      {selectedPlan ? <>
          <div className="flex items-center gap-3 pb-4 border-b border-border mb-4">
            <img src={selectedPlan.image} alt={selectedPlan.name} className="w-14 h-auto rounded-lg shadow-sm" />
            <div>
          ...
          {!isMobile && <>
              <Button type="submit" form="membership-form" onClick={onPayNow} disabled={isProcessing} className="w-full gradient-accent text-accent-foreground rounded-full py-3 text-sm font-semibold disabled:opacity-50 gap-2 mb-3">
                {isProcessing ? <>Processing...</> : <>
